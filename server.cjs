...
server.post('/ordensVenda', (req, res) => {
  ...
  if (rawKey && !idempotencyKey) {
    return res.status(400).json({ error: 'Idempotency-Key inválido (1-128 chars, alfanumérico/underscore/hífen).' });
  }
  const cacheKey = idempotencyKey ? canonicalKey(req.method, req.path, req.body, validatorUser(req)) : null;
  if (cacheKey) {
  ...
  if (!isUUID(body.clienteId) || !isUUID(body.transporteId) || !Array.isArray(body.itens) || body.itens.length === 0) {
    return res.status(400).json({
      error: 'clienteId, transporteId (ambos UUID) e ao menos 1 item são obrigatórios',
    });
  }
  ...
  for (const it of body.itens) {
    if (!isUUID(it.itemId)) {
      return res.status(400).json({ error: `itemId inválido: ${it.itemId}` });
    }
    const item = router.db.get('itens').find({ id: it.itemId }).value();
    if (!item) {
      return res.status(400).json({ error: `Item não encontrado: ${it.itemId}` });
    }
  }
...
server.patch('/ordensVenda/:id', (req, res) => {
  ...
  if (req.body.janelaAtendimento) {
    ...
    if (h1 > 23 || m1 > 59 || h2 > 23 || m2 > 59 || h2 * 60 + m2 <= h1 * 60 + m1)
      return res
        .status(400)
        .json({ error: 'janelaAtendimento inválida: horários fora do range ou fim antes do início.' });
  }
  ...
  if (auditName && !AUDIT_EXCLUDE.includes(entity)) {
    ...
    if (acao) {
      ...
      const detalhes =
        method === 'POST'
          ...
          : method === 'DELETE'
            ...
            : `${auditName.charAt(0).toUpperCase() + auditName.slice(1)} ${data?.nome || data?.id} alterado: ${Object.keys(
                req.body ?? {},
              )
                .filter((k) => k !== 'id')
                .join(', ')}`;
      ...
      const evento = {
        ...
        dataHora: new Date().toISOString(),
        detalhes,
        estadoAnterior: method === 'DELETE' ? estadoAnterior : estadoAnterior ? JSON.stringify(estadoAnterior) : null,
        estadoPosterior: method === 'DELETE' ? null : JSON.stringify(method === 'PATCH' ? req.body : data),
      };
