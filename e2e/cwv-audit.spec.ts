...
async function collectMetrics(page: Page) {
  return page.evaluate(() => {
    ...
    const metrics: Record<string, number> = {
      ...
      FCP: (fcp as PerformanceEntry | undefined)?.startTime ?? 0,
    ...
    const lcpObs = new PerformanceObserver((list) => {
      ...
      const lastEntry = last as
        (PerformanceEntry & { renderTime?: number; loadTime?: number; startTime?: number }) | undefined;
      metrics.LCP = lastEntry?.renderTime ?? lastEntry?.loadTime ?? lastEntry?.startTime ?? 0;
    });
