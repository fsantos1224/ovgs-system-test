...
export function OVList() {
  ...
  const filters = {
    ...
    nomeCliente_like: filtroCliente || undefined,
    nomeTransporte_like: filtroTransporte || undefined,
    dataEntregaPrevista_gte: dataDe || undefined,
    dataEntregaPrevista_lte: dataAte || undefined,
  };

  const { data: ordensData, isLoading } = useOrdensVenda({
    page,
    pageSize: 10,
    filters,
    sort: sort.field,
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border pb-4 md:pb-6">
        <div>
          <Breadcrumbs />
          <h1 className="text-2xl md:text-4xl font-serif italic tracking-tight text-text mt-1">Ordens de Venda</h1>
      ...
      <div className="bg-surface rounded-xl border border-border overflow-hidden shadow-2xl">
          ...
          <table className="w-full text-left border-collapse table-fixed">
            <thead>
              <tr className="bg-surface-elevated/20 border-b border-border text-[10px] font-bold text-text-faint uppercase tracking-widest">
                <th className="px-4 py-4 w-[14%] cursor-pointer select-none" onClick={() => toggleSort('numero')}>
                  Número{sort.field === 'numero' ? (sort.order === 'asc' ? ' ↑' : ' ↓') : ''}
                </th>
                <th className="px-4 py-4 w-[18%] cursor-pointer select-none" onClick={() => toggleSort('nomeCliente')}>
                  Cliente{sort.field === 'nomeCliente' ? (sort.order === 'asc' ? ' ↑' : ' ↓') : ''}
                </th>
                <th
                  className="px-4 py-4 w-[20%] cursor-pointer select-none"
                  onClick={() => toggleSort('nomeTransporte')}
                >
                  Transporte{sort.field === 'nomeTransporte' ? (sort.order === 'asc' ? ' ↑' : ' ↓') : ''}
                </th>
                <th className="px-4 py-4 w-[14%] cursor-pointer select-none" onClick={() => toggleSort('status')}>
                  Status{sort.field === 'status' ? (sort.order === 'asc' ? ' ↑' : ' ↓') : ''}
                </th>
                <th
                  className="px-4 py-4 w-[14%] text-right cursor-pointer select-none"
                  onClick={() => toggleSort('valorTotal')}
                >
                  Valor Total{sort.field === 'valorTotal' ? (sort.order === 'asc' ? ' ↑' : ' ↓') : ''}
                </th>
                <th
                  className="px-4 py-4 w-[12%] cursor-pointer select-none"
                  onClick={() => toggleSort('dataEntregaPrevista')}
                >
                  Previsão{sort.field === 'dataEntregaPrevista' ? (sort.order === 'asc' ? ' ↑' : ' ↓') : ''}
                </th>
              ...
              {ordens?.length === 0 ? (
                ...
                ordens?.map((ov) => (
                  <tr key={ov.id} className="hover:bg-hover transition-colors duration-150">
                    <td className="px-4 py-4 font-bold text-text font-mono truncate">{ov.numero}</td>
                    <td className="px-4 py-4 text-text-muted font-medium truncate">{ov.nomeCliente}</td>
                    <td className="px-4 py-4 text-text-subtle truncate">{ov.nomeTransporte}</td>
                    ...
                    <td className="px-4 py-4 text-text-subtle font-mono truncate">
                      <span className="inline-flex items-center gap-1.5">
                        <Calendar className="w-3.5 h-3.5 text-text-faint shrink-0" aria-hidden="true" />
        ...
        <div className="md:hidden divide-y divide-border-subtle">
          {ordens?.length === 0 ? (
            <div className="px-6 py-12 text-center text-text-subtle italic">Nenhuma ordem encontrada.</div>
          ) : (
            ordens?.map((ov) => (
              <div key={ov.id} className="p-4 space-y-3 hover:bg-hover transition-colors active:bg-hover-strong">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-text-muted font-medium mt-0.5 truncate">{ov.nomeCliente}</p>
                  </div>
                  <span
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-[9px] uppercase tracking-wider text-text-faint">Transporte</span>
                    <p className="text-text-subtle mt-0.5 truncate">{ov.nomeTransporte}</p>
                  </div>
                  <div>
                  ...
                  <div className="col-span-2">
                    ...
                    <p className="text-text-subtle font-mono mt-0.5">{formatDate(ov.dataEntregaPrevista)}</p>
                  </div>
                </div>
