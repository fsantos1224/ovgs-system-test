...
function applyTheme(theme: Theme) {
  ...
  localStorage.setItem("ovgs:theme", theme);
}

function getInitialTheme(): Theme {
  const stored = localStorage.getItem("ovgs:theme");
...
export function AppLayout() {
  ...
  const navItems = [
    ...
    { to: "/ovs", label: "Ordens de Venda", show: canSeeOVs, Icon: ClipboardList },
    { to: "/agendamento", label: "Agendamento", show: canSeeAgendamento, Icon: Calendar },
    { to: "/cadastros/clientes", label: "Clientes", show: canSeeClientes, Icon: Users },
    {
    ...
    { to: "/cadastros/itens", label: "Itens", show: canSeeItens, Icon: Package },
    { to: "/auditoria", label: "Auditoria", show: canSeeAuditoria, Icon: History },
  ];
  ...
  return (
    <div className="flex h-screen bg-canvas">
      ...
      >
        ...
        >
          {!collapsed && (
            <div className="flex flex-col leading-tight">
              <span className="text-[9px] tracking-[0.3em] font-bold uppercase text-text-faint">OVGS</span>
              <span className="font-serif italic text-text">Gestão</span>
            </div>
          )}
          <div className={`flex items-center gap-1 ${collapsed ? "ml-auto" : "ml-auto"}`}>
            <button
              onClick={toggleTheme}
        ...
        <nav aria-label="Menu principal" className="flex-1 p-2 space-y-1 overflow-y-auto">
      ...
      <main id="main-content" className="flex-1 overflow-auto p-8 bg-canvas" role="main">
        <Outlet />
      </main>
    </div>
  );
